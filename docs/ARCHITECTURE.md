...
- `IYoloInferenceSession` / `OnnxCudaInferenceSession`：封装 ONNX Runtime `InferenceSession` 和模型 metadata，让 `YoloDetector` 能在单元测试中注入 fake session，避免默认测试依赖真实 GPU 或模型。
...
- `YoloPostprocessor`：解析 YOLOv8/YOLO11 检测/OBB 输出，做置信度过滤、类别解析、坐标还原和 NMS；OBB 先使用水平框做 NMS，并把旋转框作为附加结果返回。
...
## GPU 策略
创建会话时优先使用 CUDA provider；如果初始化失败，必须通过异常、日志或 `ExecutionMode` 明确暴露 CPU fallback，避免调用方误判性能和部署状态。

## 扩展点
- 后续可通过新增 runtime factory 支持 DirectML，但不改变 `YoloDetector` 的主要使用方式。
- 后续可新增 `IYoloPostprocessor` 支持分割、姿态等不同模型族。
